...
if (isset($_GET['ajoutdeprod'])) {
    //permet d'extraire les champs du tableau $_GET pour simplifier
    extract($_GET, EXTR_OVERWRITE);
    if (empty($nom) || empty($id_type_produit) || empty($prix) || empty($description) || empty($image)) {
        $erreur = "Veuillez remplir tous les champs";
    } else {
        $prod = new ProduitDB($cnx);
        $prod->addProduit($_GET);
        ?>
...
<div class="row">
    ...
    <div class="col-sm-6">
        <form id="formulaireajprod">
            ...
            <button type="submit" class="btn btn-success" name="ajoutdeprod" id="ajoutdeprod">Ajouter le produit</button>
            <div>
                <?php
                if (isset($erreur)) {
                    print $erreur;
